...
World::World(QGraphicsScene* scene, QtBox2DEngine* engine, QGraphicsView* view): _scene(scene), _engine(engine), _view(view) {
  ...
  b2Body* sidewall_body = engine->createBody(b2_staticBody, 0, 0, 0, false);
  addLine(sidewall_body, scene, engine,0,0,0,10);
  addLine(sidewall_body, scene, engine,stalagtites.size()-1,0,stalagtites.size()-1,10);
